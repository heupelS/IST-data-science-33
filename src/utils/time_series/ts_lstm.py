...
def lstm_train(    
    ...
    return best[0], best_model



def lstm_plot_diagnostics(nr_features, hidden_units, learning_rate, file_tag):
    pred = DS_LSTM(input_size=nr_features, hidden_size=hidden_units, learning_rate=learning_rate)
    model = pred.fit(method_kwargs={'warn_convergence': False})
    model.plot_diagnostics(figsize=(2*HEIGHT, 2*HEIGHT))
    savefig( os.path.join(get_plot_folder_path(), f'{file_tag}_lstm_diagnostic' ) )
...
def lstm_forecast(
    ...
    trainY.index = train.index[length+1:]
    trainY.columns = [target]
    trnX, trnY  = Variable(Tensor(trnX)), Variable(Tensor(trnY))
    prd_trn = model.predict(trnX)
    prd_trn = DataFrame(prd_trn)
    prd_trn.index=train.index[length+1:]
    prd_trn.columns = [target]

    tstX, tstY = sliding_window(test, seq_length = length)
    testY = DataFrame(tstY)
    testY.index = test.index[length+1:]
    testY.columns = [target]
    tstX, tstY  = Variable(Tensor(tstX)), Variable(Tensor(tstY))
    prd_tst = model.predict(tstX)
    prd_tst = DataFrame(prd_tst)
    prd_tst.index=test.index[length+1:]
    prd_tst.columns = [target]

    plot_evaluation_results(trnY.data.numpy(), prd_trn, tstY.data.numpy(), prd_tst, f'{file_tag}_lstm_eval.png')
    savefig( os.path.join(get_plot_folder_path(), f'{file_tag}_lstm_eval' ) )

    plot_forecasting_series(trainY, testY, prd_trn.values, prd_tst.values, f'{file_tag}_lstm_plots.png', x_label=index, y_label=target)
    savefig( os.path.join(get_plot_folder_path(), f'{file_tag}_lstm_forecast' ) )
